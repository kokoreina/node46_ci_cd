...
export const roleService = {
   ...
   findAll: async function (req) {
        ...
        const roles =await prisma.roles.findMany({
            ...
            skip : skip,
            orderBy :{
                created_at :`desc`, // sắp xếp giảm dần: đưa video mới nhất lên đầu
            },
            where : where
        })
        ...
        return {
            page, // trang hiện tại
            pageSize, // kích thước item trong 1 page : 5 video trong 1 page
   ...
   findOne: async function (req) {
      ...
      const role =await prisma.roles.findUnique({
        where :{
            role_id : +id,// Thêm dấu + để chuyển từ chuỗi sang number vì dữ liệu gửi ở param luôn là chuỗi
        }
      })
      return role;
   },
   ...
   togglePermission: async (req) => {
    ...
    console.log(3,{rolePermissionExists})
